...
def _preflight_model_check(
    ...
    *,
    ...
    """
    if os.getenv("TOOL_EVAL_MIRROR_BASE_URL", "").strip():
        # The first real scenario is the deliberate seam test for mirrored
        # external-launcher TP.  A rank-0-only synthetic preflight would block
        # its peer in a collective and adds no decision-relevant evidence.
        return
...
async def _stderr_progress_result(
    ...
    # A full Standard69/Hard15 pass can run for hours.  Preserve each scored
    # scenario as soon as it finishes so a server failure never discards the
    # preceding work.  The main report is still written normally at the end.
    progress_path = os.getenv("TOOL_EVAL_PROGRESS_JSONL", "").strip()
    if progress_path:
