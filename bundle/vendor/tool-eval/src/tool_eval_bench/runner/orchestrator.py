...
async def run_scenario(
    ...
    *,
    ...
    initial_tool_choice = scenario.tool_choice_override or "auto"
    available_tool_names = [
        str(tool.get("function", {}).get("name", "?"))
        for tool in (scenario_tools or [])
    ]
    trace_lines: list[str] = [
        "assistant=starting",
    ...
    if available_tool_names:
        trace_lines.append(f"tool_choice={initial_tool_choice}")
    ...
    try:
        for turn in range(1, max_turns + 1):
            # Stream every turn so a genuine exact-repeat loop can close the
            # request immediately instead of consuming the remaining context.
            use_stream = True

            turn_start = time.perf_counter()
            scenario_tool_choice = initial_tool_choice
            if scenario_tool_choice == "required" and state.tool_calls:
                # "required" is a per-request constraint. After the scenario has
                # observed a valid tool call, keep tools available without forcing
                # every follow-up response to call another tool.
                scenario_tool_choice = "auto"
            ...
            result = await adapter.chat_completion(
                ...
                # None means the backend may use the full remaining model
                # context.  Do not impose a benchmark-side short cap.
                max_tokens=None,
            ...
            if result.raw_response.get("loop_aborted"):
                trace_lines.append(
                    "exact_loop_aborted="
                    f"period_chars:{result.raw_response.get('loop_period_chars')},"
                    f"observed_output_chars:{result.raw_response.get('observed_output_chars')}"
                )
