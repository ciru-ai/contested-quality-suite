...
def main(
    ...
    if os.environ.get("AIDER_PORTABLE_SOURCE_REV"):
        commit_hash = os.environ["AIDER_PORTABLE_SOURCE_REV"][:7]
    else:
...
def run_unit_tests(original_dname, testdir, history_fname, test_files):
    ...
    # Local-custom limit review: preserve full test output, bound only feedback.
    raw_path = testdir / (".test-output-" + str(time.time_ns()) + ".txt")
    raw_path.write_text(res)
    if len(res) > 32000:
        original_chars = len(res)
        res = res[:24000] + "\n[Repeated/large test output omitted; original chars=" + str(original_chars) + "; full log=" + raw_path.name + "]\n" + res[-8000:]
    dump(res)
